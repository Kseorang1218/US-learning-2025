...
def main(config):
    ...
    model = funs.AE(input_dim=config.sample_size, latent_dim=32).to(device)
    optimizer = Adam(model.parameters(), lr = float(config.learning_rate))
    loss = torch.nn.MSELoss()

    # train 
    trainer = funs.TrainerAE(model, loss, optimizer, device)
    # train_loss_list = trainer.train(config.epoch, train_loader)
    # trainer.save(config.model_root, model_name='ae')

    model_path = f'{config.model_root}/ae.pt'
